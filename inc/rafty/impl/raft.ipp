...
inline void Raft::start_server() {
  ...
  auto service=std::make_unique<RaftServiceImpl>(this);
  builder.RegisterService(service.get()); /* (replace nullptr) */
...
inline void Raft::kill() {
  ...
  if (this->main_loop_thread_.joinable()) this->main_loop_thread_.join();
  for (auto &t: this->heartbeat_threads_) {
    if (t.joinable()) t.join();
  }
  this->heartbeat_threads_.clear();
  for (auto &t: this->election_threads_) {
    if (t.joinable()) t.join();
  }
  this->election_threads_.clear();
